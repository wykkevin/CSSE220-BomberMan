...
public class Bomb implements Runnable {
		...
		for (int i = 0; i < 50 * this.range + 1; i++) {
			if ((((this.getPosition().x + x * i) / 50) < 14) && (((this.getPosition().x + x * i) / 50) >= 0)
				...
				if (currentMap[(this.getPosition().x + x * i) / 50][(this.getPosition().y + y * i) / 50] == 1) {
					...
					} else if (randNum >= 0.65 && this.hero.getBombUpgrade() == false) {
	...
	public void run() {
		int count = 200;
		while (count >= 0 && !this.hero.getBombUpgrade()) {
			try {
		...
		for (int i = this.position.x / 50 - this.range; i < this.position.x / 50 + this.range + 1; i++) {
			for (int j = this.position.y / 50 - this.range; j < this.position.y / 50 + this.range + 1; j++) {
